...
* Далее создается фрейм метода main()
    ...
    * после этого фрейм printAll() удаляется из стека из стека как завершенный
    ...
    - после этого фрейм System.out.println() удаляется из стека как завершенный
* И так далее в обратном порядке будут удалены все фреймы (сверху в низ), в куче остаются три объекта: два Integer и Object, которые будут удалены при очередном сборе мусора.
...
* При первом и последнем создании 5млн объектов размер кучи был увеличен.
